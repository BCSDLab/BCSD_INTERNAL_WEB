...
import { Button, Checkbox, FormControl, FormControlLabel, FormGroup, FormLabel, Input, Popover } from '@mui/material';
import Modal from "@mui/material/Modal";
import { ChangeEvent, useState } from 'react';
import { useGetAllDues } from 'query/dues.ts';
import useBooleanState from 'util/hooks/useBooleanState.ts';
import { DuesDetail } from 'model/dues/allDues.ts';
import { ALL_TRACKS } from 'util/constants/alltracks.ts';
...
export default function DuesManagement() {
  ...
  const [detail, setDetail] = useState<DuesDetail>({ month: 0, status: "" });
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  const memoPopOverOpen = Boolean(anchorEl);
  const { value: isFilterModalOpen, setTrue: openFilterModal, setFalse: closeFilterModal } = useBooleanState(false);
  ...
  const handleTrackFilterChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setTrack((prevTrack) => {
      ...
      setFilteredValue(allDues.dues.filter((row) => updatedTrack[ALL_TRACKS.indexOf(row.track.name)]));
      return updatedTrack;
    });
  }

  const handleMemoClick = (e: React.MouseEvent<HTMLTableCellElement>, detail: DuesDetail) => {
    if (detail.status === "미납" || detail.status === "면제") {
  ...
  return (
    <div css={S.container}>
      ...
      <div css={S.content}>
        <div css={S.topBar}>
          <h1 css={S.topBarTitle}>{duesYear}년 회비 내역</h1>
        </div>
                ...
                <TableHead>
                  <TableRow>
                    ...
                    {Array.from({ length: 12 }, (_, i) => i + 1).map((month) => (
                      <TableCell key={month} css={S.tableHeader}>{month}월</TableCell>
                    ))}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {filteredValue.map((row) => (
                    ...
                    >
                      ...
                      {row.detail.map((detail) => (
                        <TableCell
                          css={S.memoTableCell({ status: detail.status})}
                          onClick={(e) => handleMemoClick(e, detail)}
                          key={detail.month}
                        >
                          {/* TODO: detail.status에 따른 UI */}
                          {/* 미납 X(빨강), 면제 -(초록), 납부 O(초록), null -(default) */}
                          {detail.status === "미납" ? "X" : detail.status === "면제" ? "-" : detail.status === "납부" ? "O" : "-"}
                        </TableCell>
                      ))}
                    </TableRow>
                  ))}
